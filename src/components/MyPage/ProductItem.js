      ...
      <Animated.View style={[styles.productItem, animatedStyle]}>
        <View style={styles.productItem}>
          ...
          <View style={{ overflow: "hidden", marginRight: 50 }}>
            <Text style={{ color: "blue", fontSize: 11, marginBottom: 3 }}>
              {item?.category}
            </Text>
            <Text
              numberOfLines={1}
            ...
            >
              {item.isdiscounting
                ? item.discountprice.toLocaleString("ko-kr")
                : item.price?.toLocaleString("ko-kr")}
              원 | 재고: {item.inventory}
            </Text>
            <Text style={{ color: item.status === "판매중" ? "black" : "red" }}>
              {item.status}
            </Text>
          </View>
